...
const SendMail = async (email: string, tokenSenha: string) => {
  ...
  // Modificado para ler configurações de e-mail da tabela Settings
  const mailHost = await Setting.findOne({ where: { key: "mail_host" } });
  const mailUser = await Setting.findOne({ where: { key: "mail_user" } });
  const mailPass = await Setting.findOne({ where: { key: "mail_pass" } });
  const mailFrom = await Setting.findOne({ where: { key: "mail_from" } });
  const mailPort = await Setting.findOne({ where: { key: "mail_port" } });

  if (!mailHost || !mailUser || !mailPass || !mailFrom || !mailPort) {
    console.error("Configurações de e-mail não encontradas na tabela Settings");
    return { status: 500, message: "Erro nas configurações de e-mail" };
  }
  ...
  // Fim da modificação para configurações de e-mail
  ...
  if (hasResult === true) {
    ...
    async function sendEmail() {
      try {
        const mailOptions = {
          from: mailFrom.value, // Modificado para usar mail_from da tabela Settings
          to: email,
                 ...
                 <tr>
                  <td align="center" valign="top" style="padding:0;Margin:0;width:540px">
                   <table cellpadding="0" cellspacing="0" width="100%" role="presentation" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px">
                     <tr>
                      <td align="center" style="padding:0;Margin:0;font-size:0px"><img src="https://comercial.idemaisag.com.br/public/logotipos/logo.png" alt style="display:block;border:0;outline:none;text-decoration:none;-ms-interpolation-mode:bicubic" width="205"></td>
                     </tr>
                   </table></td>
                 </tr>
               </table></td>
                 ...
                 <tr>
                  <td align="center" valign="top" style="padding:0;Margin:0;width:560px">
                   <table cellpadding="0" cellspacing="0" width="100%" role="presentation" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px">
                     ...
                     <tr>
                      <td align="center" style="padding:0;Margin:0;padding-top:10px;padding-bottom:10px"><p style="Margin:0;-webkit-text-size-adjust:none;-ms-text-size-adjust:none;mso-line-height-rule:exactly;font-family:roboto, 'helvetica neue', helvetica, arial, sans-serif;line-height:24px;color:#131313;font-size:16px">Você solicitou a recuperação de senha do sistema?</p></td>
                     </tr>
                   </table></td>
                 </tr>
               </table></td>
